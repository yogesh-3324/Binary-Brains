...
def process_reference_pool(folder_path):
    ...
    # 2. Setup DB Paths
    save_dir = "dinov2_pinecone_store"
    ...
    if os.path.exists(path_vectors) and os.path.exists(path_paths) and os.path.exists(path_hashes):
        try:
            ...
            print(f"Loaded existing index cache with {len(old_paths)} images.")
        except Exception as e:
            print(f"Could not load existing index cache ({e}). Starting fresh.")
    ...
    if not files_to_process:
        print("Index is already up to date in Pinecone. No new images to add.")
    ...
    # 6. Upsert vectors and metadata to Pinecone Vector DB
    print(f"Upserting {len(files_to_process)} new image vectors to Pinecone...")
    pinecone_store = get_pinecone_store()
    pinecone_store.upsert_image_vectors_batch(files_to_process, new_vectors, new_hashes)

    # 7. Merge Old + New Data for local cache
    ...
    print("Pinecone Vector Database updated successfully.")
...
def remove_image_from_index(filename, store_dir="dinov2_pinecone_store"):
    print(f"Attempting to remove '{filename}' from Pinecone vector store...")
    ...
    try:
        pinecone_store = get_pinecone_store()
        pinecone_store.delete_by_filename(filename)
    except Exception as e:
        print(f"Warning deleting vector from Pinecone: {e}")
    ...
    try:
        ...
        if idx_to_remove != -1:
            print(f"Found at index {idx_to_remove}. Removing from local cache...")
        ...
        print(f"Error updating local metadata: {e}")
        return False

def add_image_to_index(image_path, store_dir="dinov2_pinecone_store"):
    print(f"Adding new image to Pinecone index: {image_path}")
    ...
    # Embed image
    new_vector = embed_images_batch([image_path], batch_size=1)
    ...
    pinecone_store = get_pinecone_store()
    pinecone_store.upsert_image_vector(image_path, new_vector[0], new_hash)
    ...
    print("Image added successfully to Pinecone and metadata updated.")
    return True
